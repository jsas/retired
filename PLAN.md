...
| 64 | Point Pages base at /retired/ + release CI + v0.1.0 | ✅ | done 2026-08-24 — vite base → /retired/; version → 0.1.0 (pkg+lock); deploy.yml gains a tag-triggered release job (softprops/action-gh-release attaching single-file HTML + zipped site); v0.1.0 annotated tag created. Committed (0b2eee8 + PLAN 5286559). User handles push (`git push -u origin main` + `git push origin v0.1.0`) + repo Settings → Pages → GitHub Actions |
