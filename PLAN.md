...
| 64 | Point Pages base at /retired/ + release CI + v0.1.0 | 🔄 | done locally 2026-08-24 — vite base → /retired/; version → 0.1.0 (pkg+lock); deploy.yml gains a tag-triggered release job (softprops/action-gh-release attaching single-file HTML + zipped site); v0.1.0 annotated tag created. Committed as 0b2eee8. Awaiting user push (`git push -u origin main` + `git push origin v0.1.0`) and repo Settings → Pages → GitHub Actions |
