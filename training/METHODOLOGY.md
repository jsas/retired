...
|---|---|---|---|
| 1 | **Drive the tools** — emit ONE clean `<tool_call>` block with valid args | `tool-call` | malformed/hallucinated calls, multi-call spam |
...
all produce the *identical* canonical `<tool_call>` block. "Am I on track?", "Will my
...
## 3. How the training run reinforces it (LLaMA-Factory)

The data encodes the behavior; the training config makes it stick. Training runs
through **LLaMA-Factory**, driven by a single YAML — no bespoke trainer. Two
recipes ship: `lf_lora.yaml` (fast validation) and `lf_sft.yaml` (the ship
candidate). See [USAGE.md](./USAGE.md) §4 for how to launch them.

**Protocol is Qwen3-native, rendered by the chat template.** The corpus carries
`<tool_call>{"name","arguments"}</tool_call>` blocks (`training/protocol.ts`).
`toLlamaFactory.ts` converts these to sharegpt `function_call`/`observation`
roles, and the `qwen3_nothink` chat template renders them back through Qwen3's
own special tokens — so the model learns tool-calling in the format its weights
were pre-trained on, not a hand-rolled prefix it has to learn from scratch. The
runtime parser accepts both native blocks and the legacy `TOOL_CALL:` line.

**Mask the loss to assistant tokens.** Still the single most important knob, and
LLaMA-Factory's sharegpt SFT does it by construction: loss is computed only on
the `gpt`/`function_call` turns. The model is rewarded for producing *its own*
turns (the tool call, the grounded explanation, the neutral framing) — never for
predicting the user's question or the system prompt. That's what makes the
reward signal "be a good assistant," not "parrot the transcript."

**Assistant-token-only loss is *how* "rewards being good" is implemented.** SFT
has no separate reward model — the reward is "match the target assistant turn."
So the quality bar lives entirely in the *targets*: every assistant turn in the
corpus is a vetted example of the behavior in §1. Clean targets → the model is
rewarded for exactly the right thing. Garbage targets → it learns garbage. That's
why the corpus is generated by code against the live catalog
...
**The tool manifest is slimmed, deliberately.** The full JSON schemas ran ~6.2k
tokens — larger than the training window — so every example truncated
mid-manifest and the model never saw the conversations. The converter emits a
~2.1k-token slim manifest (name, short description, arg names/types/required,
enum values). That's everything the model needs to emit a valid call; the full
schemas stay in `src/lib/ai/tools.ts` for runtime validation. This single change
took full examples from "never fits" to "max 3.2k tokens," which is also what
made training tractable on a 16 GB card.

**LoRA first, then full SFT — decide on the number.** Methodology §5 of SPIKE
argued full-SFT-first for structured-output reliability. In practice, on a
memory-bandwidth-bound 5070 Ti, full SFT at 3.5k context runs ~40h/epoch while
LoRA runs ~15h. So the loop is: **validate the whole pipeline (data → train →
eval → export) on the cheap LoRA pass first**, and only commit the 40h full run
once the LoRA gate score tells us whether adapters suffice for the format. LoRA
is the scouting run, not the ship candidate — unless its protocol-validity
already clears the bar, in which case we've saved 25h.

**Hyperparameters live in the YAMLs,** tuned per recipe: LoRA wants a higher LR
(`1e-4`, rank 16, alpha 32, all-linear target) because only adapters train; full
SFT uses `5e-6`. Both share bf16, `cutoff_len 3584`, packing, cosine schedule,
and `plot_loss` for the live TensorBoard curves. **Early-stop / ship decision is
on the protocol-validity eval**, not on vibes.
