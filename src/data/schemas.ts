 ...
 * Legacy payloads (pre-schema fields, bare plan arrays) are run through
...
export function parseAppDbDoc(raw: unknown): AppDbDoc | null {
  ...
  // Migrate each plan's inputs first (fills fields added by later
  // versions), then validate the result strictly.
  if (Array.isArray(candidate.plans)) {
    for (const s of candidate.plans) {
  ...
  // An active id that no longer exists falls back to the first plan.
  if (!result.data.plans.some(s => s.id === result.data.activePlanId)) {
    result.data.activePlanId = result.data.plans[0].id;
  }
  return result.data;
}
