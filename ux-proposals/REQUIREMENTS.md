...
**Status:** requirements · **Date:** 2026-08-30
**Scope:** the app's core UX language — verdict, controls, and how the dashboard is organized.

> The 40 candidate mockups and their ratings that produced these requirements
> were pruned once the winner was chosen. The surviving artifact is the winning
> design, **f7 · The Final** (`finalists/f7-final*.html`), plus the shared
> `model.js` every mock computed through. The full set of rejected candidates
> remains on the `makeover-proposals` branch for reference.
