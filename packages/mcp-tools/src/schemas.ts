 ...
 * assistant tools exchange (plan inputs, spouse, accounts, income, events,
 * debts, benefits, engine config). These are the PORTABLE schemas: they depend
 * only on @retired/engine-core types, so both the web app and the MCP server
 * validate against the same definitions. (The app-only whole-database document
 * schema and its input migration stay in the app's data/schemas.ts.)
 *
